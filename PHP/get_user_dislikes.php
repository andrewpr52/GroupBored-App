 ...
 */
	...
	if (isset($_GET['username'])) {
		...
		$posts = mysqli_query($con, "SELECT * FROM Posts JOIN Dislikes ON (Posts.ID = Dislikes.PostID) WHERE Dislikes.UserID = '$uID'");
		...
		if (!empty($posts)) {
			...
			if (!empty($rows)) {
				...
				$response["success"] = 0;
				$response["message"] = "No disliked posts for user.";
